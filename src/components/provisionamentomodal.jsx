          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            <Field label="Use Case">
              <select style={inputStyle} value={form.id_usecase_ai} onChange={e => handleChange('id_usecase_ai', e.target.value)}>
                <option value="">— selecionar —</option>
                {(form.id_iniciativa ? useCaseOptions : Array.from(new Map(iniciativas.map(r => [String(r.id_use_case), r.nome_use_case])).entries()).map(([id, nome]) => ({ id, nome }))).map(o => <option key={o.id} value={o.id}>{o.id} — {o.nome}</option>)}
              </select>
            </Field>
          </div>
